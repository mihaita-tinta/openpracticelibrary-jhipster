...
type RestOf<T extends IAuthor | NewAuthor> = Omit<T, 'publishDate' | 'approvedDate'> & {
...
export class AuthorService {
  ...
  protected convertDateFromClient<T extends IAuthor | NewAuthor | PartialUpdateAuthor>(author: T): RestOf<T> {
    return {
      ...author,
      publishDate: author.publishDate?.toJSON() ?? null,
      approvedDate: author.approvedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAuthor: RestAuthor): IAuthor {
    return {
      ...restAuthor,
      publishDate: restAuthor.publishDate ? dayjs(restAuthor.publishDate) : undefined,
      approvedDate: restAuthor.approvedDate ? dayjs(restAuthor.approvedDate) : undefined,
    };
  }
