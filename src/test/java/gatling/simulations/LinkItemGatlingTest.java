...
public class LinkItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all linkItems").get("/api/link-items").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new linkItem")
                        .post("/api/link-items")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"url\": \"SAMPLE_TEXT\"" + ", \"sortIndex\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_linkItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created linkItem").get("${new_linkItem_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created linkItem").delete("${new_linkItem_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LinkItem entity").exec(scn);
