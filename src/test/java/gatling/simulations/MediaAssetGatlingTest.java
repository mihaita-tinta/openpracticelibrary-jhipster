...
public class MediaAssetGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all mediaAssets").get("/api/media-assets").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mediaAsset")
                        .post("/api/media-assets")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"type\": \"IMAGE\"" + ", \"content\": null" + ", \"sortIndex\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mediaAsset_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created mediaAsset").get("${new_mediaAsset_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created mediaAsset").delete("${new_mediaAsset_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MediaAsset entity").exec(scn);
