...
public class PracticeItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all practiceItems").get("/api/practice-items").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new practiceItem")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"why\": null" +
                                ", \"how\": null" +
                                ", \"numberOfPeopleRequired\": \"SAMPLE_TEXT\"" +
                                ", \"timeLength\": \"SAMPLE_TEXT\"" +
                                ", \"expectedParticipants\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_practiceItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created practiceItem").get("${new_practiceItem_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created practiceItem").delete("${new_practiceItem_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PracticeItem entity").exec(scn);
